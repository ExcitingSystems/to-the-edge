...
class RemoteDynamicComposite(DynamicComposite):
    def __init__(self,
        ...
        # = 1t + observation_length + 1idx + 1reset
        self.measurement_length = 3 + self.observation_length

        # define entries of received data
        self.time_idx = 0
        self.stateCurrent_idx = 1
        self.trajectoryIdx_idx = 3
        self.trajectoryReset_idx = 4

        # buffer size is in bytes, float has size of 4 bytes:
        self.data_send_buffer_size = int(
            1024 // (self.measurement_length * 4)) * self.measurement_length * 4
    ...
    def start(self, verbose=0, callbacks=None):
        ...
        data_communication = threading.Thread(
            target=self._recv_data, args=(verbose,))
    ...
    def _recv_data(self, verbose=0, log_interval=10000):
        ...
        while not self.close_agent:
            ...
            while select.select([self.data_conn], [], [], 0.0)[0]:
                ...
                full_dates_len = (
                    data_len // self.measurement_length) * self.measurement_length
                if local_buffer is None and data_len % self.measurement_length:
                    ...
                    try:
                        ...
                        raise
                try:
                    episode_data = np.reshape(
                        float_data, (self.measurement_length, -1), order="F")  # seems to be fine
                except:
                    print(float_data)
                    raise
                ...
                for _i in range(len(episode_data[self.time_idx])):
                    self.memory.append(episode_data[self.observation_idx:self.action_idx, _i],  # state
                                       (episode_data[self.action_idx, _i]).astype(
                                           int),  # action
                                       # reward
                                       episode_data[self.reward_idx, _i],
                                       # done
                                       bool(
                                           episode_data[self.doneflag_idx, _i]),
                                       {},
                ...
                if select.select([self.weights_conn], [], [], 0.0)[0]:
                    ...
                    for _i, _layer_dim in enumerate(self.architecture):
                        ...
                        if (len(b) < self.weightBufferSize):
                            ...
                            for i in range(0, len(b) // self.weightBufferSize):
                                self.weights_conn.send(
                                    b[i * self.weightBufferSize:(i + 1) * self.weightBufferSize])

                            if len(b) > ((i + 1) * self.weightBufferSize):
                                self.weights_conn.send(
                                    b[(i + 1) * self.weightBufferSize:])
        finally:
                ...
                if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
                    ...
                    if self.enable_double_dqn:
                        ...
                        assert target_q_values.shape == (
                            self.batch_size, self.nb_actions)
                        q_batch = target_q_values[range(
                            self.batch_size), actions]
                    else:
                    ...
                    if self.learning_rate != self.new_learning_rate and time.time()-self.last_print_time >= 10:
                        ...
                        K.backend.set_value(
                            self.trainable_model.optimizer.optimizer.lr, self.new_learning_rate)
                        self.learning_rate = self.new_learning_rate
                        self.last_print_time = time.time()

                    train_stop = time.time()
                    self.train_time_avg = (
                        self.train_time_avg * self.n + (train_stop-train_start)) / (self.n+1)
                    self.n += 1
